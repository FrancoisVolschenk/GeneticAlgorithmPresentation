...
class Layer:
    ...
    #     return 1.0 / 1.0 + (_E ** (-1.0 * weightedInput))

    def Activation(self, weightedInput: float) -> int:
        """The ReLu function"""
        return max(0, weightedInput)

    # def sigmoid(self, weightedInput: float) -> float:
    #     return 1/(1 + np.exp(-weightedInput)) 

    # def Activation(self, weightedInput: float) -> float:
    #     return (2 * self.sigmoid(weightedInput * 2)) - 1
